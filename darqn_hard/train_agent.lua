...
while step < opt.steps do
    ...
    if step % opt.eval_freq == 0 and step > learn_start then
        ...
        print(string.format(
            '\nSteps: %d, reward: %.2f, last: %.2f,  #ep.: %d,  #rw.: %d, epsilon: %.2f, lr: %G, ' ..
            'training: %ds/%dfps, testing: %ds/%dfps',
            step, total_reward, episode_reward,
            nepisodes, nrewards, agent_ep, agent.lr,
            time_dif, training_rate, eval_time, opt.actrep*opt.eval_steps/eval_time))
    end
